...
category: null
keywords: 用java快速高效生成大量随机不重复的字符串
description: 如何用java生成随机不重复的字符串，一个原创的高效算法
---
...
目前此算法某些情况下不是那么随机，例如在一个整数附近，例如10000左右，生成的随机数如下：
...
改进方案，针对重复的字符串，进行第二次替换，方法如下：
* 显然，只有字符串长度大于或等于3个时，才有必要替换；
* 从第3个字符依次遍历，查看当前字符是否与前一个字符重复，如果重复则将前一个字符替换；
* 替换规则为位移字符的前i个字符；

具体流程参考下图
![](http://7xvhwc.com1.z0.glb.clouddn.com/liuchengtu.gif)


第二点，上面的例子只是使用a~z小写的字母，如果区分大小写的话，还可以将大写字母也进一步纳入到游戏规则，改进方法很简单，直接将字典数组更新即可，（这里将大小写字母交叉混合以降低规律性）
